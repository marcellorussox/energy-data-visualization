...
top10_total <- energy_data %>%
  ...
  mutate(
    ...
    label_text = paste0(scales::comma(energy_per_capita), " kWh/person")  # Modifica qui
  ) %>%
...
ggplot(top10_total, aes(x = primary_energy_consumption/1000, 
  ...
  scale_x_continuous(
    name = "Total Energy Consumption (Thousand TWh)",
  ...
  labs(
    ...
    y = "",
    subtitle = "Energy per capita calculated as total consumption divided by population"
  ) +
  theme_minimal() +
  theme(
